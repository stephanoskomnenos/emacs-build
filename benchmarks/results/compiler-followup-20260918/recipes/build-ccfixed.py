...
s=s.replace("variant = 'llvm-' + profile_mode + ('-stable' if stable_locals else '')","variant = 'llvm-' + profile_mode + '-cccs'")
s=s.replace("'CFLAGS': env['CFLAGS'].replace('-fPIC', '-fPIE') + pgo_flags", "'CFLAGS': env['CFLAGS'].replace('-fPIC', '-fPIE') + ' -fprofile-use=/work/build/merged.profdata -Xclang -fprofile-instrument=csllvm'")
s=s.replace("'emit_relocs': True,", "'emit_relocs': True, 'cs_link_only': True, 'cs_runtime_marker': True, 'compile_profile': '/work/build/merged.profdata',")
