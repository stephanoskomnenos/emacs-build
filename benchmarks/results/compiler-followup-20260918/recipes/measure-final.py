...
b=Path('/work/build');names=['ordinary','ordinary-cc-control','cs-cc-fixed'];bundles={n:Path((b/(n+'-bundle')).read_text().strip()) for n in names}
for name in names[1:]:
...
(b/'final-measurement-order.json').write_text(json.dumps(order,indent=2))
for e in order:
 name=e['variant'];label=f"final-r{e['round']}-b{e['block']}-{name}"
