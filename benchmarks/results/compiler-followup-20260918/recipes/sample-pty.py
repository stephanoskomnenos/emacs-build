...
base=Path('/work/build/regexp-followup'); base.mkdir(exist_ok=True)
for kind in ['ordinary','cs-link-fixed']:
 bundle=Path('/work')/Path('/work/build/'+kind+'-bundle').read_text().strip()
