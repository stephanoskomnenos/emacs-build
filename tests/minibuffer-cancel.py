...
base=ROOT/'build/cancel-regression';(base/'home').mkdir(parents=True,exist_ok=True)
env=dict(os.environ,HOME=str(base/'home'),TRAIN_PACKAGES=str(ROOT/'build/workload-packages/paths.json'))
for key in ('LLVM_PROFILE_FILE','EMACSLOADPATH','EMACSDATA','EMACSDOC','EMACSPATH','LD_LIBRARY_PATH'):env.pop(key,None)
for affinity in [{min(os.sched_getaffinity(0))},set(os.sched_getaffinity(0))]:
    pid=os.fork()
    if pid==0:
        os.sched_setaffinity(0,affinity)
        session=Session(a.bundle.resolve()/'bin/emacs',base/str(len(affinity)),ROOT/'benchmarks/interactive/training.el',env)
        try:
            for _ in range(100):
        ...
        print(f'PASS: 100 cancellations with {len(affinity)} allowed CPUs',flush=True)
        os._exit(0)
